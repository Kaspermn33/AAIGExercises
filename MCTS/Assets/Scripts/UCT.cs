...
public class UCT : MonoBehaviour
{
    ...
    public int RunUCT()
    {
        ...
        Node bestChild = BestChild(rootNode, C);
     ...
     * @return
     */
    private float UCTvalue(Node child, Node parent, float c)
    {
        // TODO: calculate the UCT value for the node n ad return it
        return (child.reward / child.timesvisited) + c * Math.Sqrt((2*Math.Log(parent.timesvisited))/child.timesvisited);
    }
